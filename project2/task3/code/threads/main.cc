...
main(int argc, char **argv)
{
    ...
    for (i = 1,p=0; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
     //    char* tem=argv[i + 1];
	    // userProgName.push_back(tem);
        userProgName[p]=argv[i + 1];
		p++;
        // delete tem;
	    i++;
	}
	...
	for(int p=0;p<20;p++)
	{
    if (userProgName[p] != NULL) {
     Thread *t1 = new Thread(userProgName[p]);
     t1->Fork((VoidFunctionPtr) RunUserProg, (void *) userProgName[p]);
    }
	}
