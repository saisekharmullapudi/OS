...
{
    // std::cout<<kernel->currentThread->id<<"    inside Machine translate      "<<virtAddr<<endl;
    int i;
    ...
    // vpn=divRoundUp(virtAddr, PageSize)-1;
    // offset=virtAddr-vpn*PageSize;

    if (tlb == NULL) {      // => page table => vpn is index into table
        // if (vpn >= kernel->currentThread->space->numPages)
        // {
        ...
        {
            IPTEntry* t= ipt.at(i);
            if(t->Thread_id==kernel->currentThread->id && vpn==t->virtualPage)
            {
                entry=t;
                v= true;
    ...
    // std::cout<<"vpn      :"<< vpn<<"    ;ppn   :"<<ppn<<" virtAddr   :  "<<virtAddr<<"   id   "<<kernel->currentThread->id<<endl;
    if(ppn>NumPhysPages)
    {
        return PageFaultException;
    ...
    // std::cout<<"vpn      :"<< vpn<<"    ;ppn   :"<<ppn<<" virtAddr   :  "<<virtAddr<<endl;
    // std::cout<<entry->readOnly << writing<<endl;
    return ReadOnlyException;
    }
    if (ppn >= NumPhysPages) { 
    DEBUG(dbgAddr, "Illegal pageframe " << ppn);
