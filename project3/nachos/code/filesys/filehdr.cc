...
{ 
	if(kernel->segment->counter >= (NumSectors*SectorSize -1))
	{
		cout<< "Segments has been fileed OS."<<endl;
		ASSERT(false);
	}
...
//        ASSERT(dataSectors[i] != EMPTY_BLOCK);                      // assert that allocation was good
        int result = dblock->Allocate(numReqSectors - allocated);      // allocate doubly indirect block
        ASSERT(result != -1);// assert doubly indirect block allocation succeeded
        dataSectors[i] = kernel->segment->counter;
        dblock->WriteBack();                          // write doubly indirect block back
