...
Timer::SetInterrupt()
{
	//std::cout<<"infinite looping"<<endl;
	int t=0;
	int level=-1;
    ...
    if (!disable) {
	   ...
	   if(t<0)
	   {
		   //std::cout<<"error  :"<<t<<" level :"<< level<<endl;
	   }
	   if(t==0)
	   {
		   kernel->interrupt->Schedule(this, delay, TimerInt);
		   std::cout<<"timer interrutp for empty threads"<< delay<<endl;
	   }
	   else{
		   kernel->interrupt->Schedule(this, t, TimerInt);
		   t=0;
		   //std::cout<<"timer interrutp at"<< t<<endl;
	   }
       level=-1;
    }
}