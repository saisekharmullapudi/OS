...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
	//kernel->interrupt->RemoveTimeInterrupt();
	kernel->scheduler->ReadyToRun(this);
	kernel->scheduler->Run(nextThread, FALSE);
    }
	setLastExecuted__time(kernel->stats->totalTicks);
...
int Thread::increasePriority()
{
	old_level=MLFQ_level;
	if(MLFQ_level>1)
	{
		MLFQ_level=MLFQ_level-1;
	}
	else if(MLFQ_level==1)
	{
		MLFQ_level=1;
	}
	return MLFQ_level;
...
void Thread::setLastExecuted__time( int t)
{
	last_executed_Time=t;
}

int Thread ::getLastExecuted__time()
{
